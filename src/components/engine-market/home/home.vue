<template>
          ...
          <div class="blPadding">
            <div class="padList" :class="{ 'warning-alert': showWarning1 }">
              <div :class="{ active: ldTypeShow }">
                <!-- <img class="plIco" src="image/oneIco.png" /> -->
                <!-- <img class="plIco" src="/public/image/oneIco.png" /> -->
                <div class="plIco oneIco"></div>
                <span class="plName"
                  >{{
                    listData?.dew_point_c?.findIndex(
                      (item) => item.id === dew_point_cId
                    ) !== -1
                      ? dew_point_cName
                      : "未配置"
                  }}:</span
          ...
          <div class="blPadding">
            <div class="padList" :class="{ 'warning-alert': showWarning2 }">
              <div :class="{ active: wdTypeShow }">
                ...
                <span class="plName"
                  >{{
                    listData?.temperature?.findIndex(
                      (item) => item.id === temperatureId
                    ) !== -1
                      ? temperatureName
                      : "未配置"
                  }}:</span
          ...
          <div class="blPadding">
            <div class="padList" :class="{ 'warning-alert': showWarning4 }">
              <div :class="{ active: sdTypeShow }">
                ...
                <div class="plIco sd"></div>
                <span class="plName"
                  >{{
                    listData?.humidness?.findIndex(
                      (item) => item.id === humidnessId
                    ) !== -1
                      ? humidnessName
                      : "未配置"
                  }}:</span
                ...
                <span class="plNum">{{
                  filterNum(
                    listData?.humidness?.find((item) => item.id === humidnessId)
                      ?.value
                      ? listData?.humidness?.find(
                          (item) => item.id === humidnessId
                        )?.value
                      : "NA",
                    "sd"
          ...
          <div class="blPadding">
            <div class="padList" :class="{ 'warning-alert': showWarning3 }">
              <div :class="{ active: ycTypeShow }">
                ...
                <div class="plIco yc"></div>
                <span class="plName"
                  >{{
                    listData?.dif_pressure_pa?.findIndex(
                      (item) => item.id === dif_pressure_paId
                    ) !== -1
                      ? dif_pressure_paName
                      : "未配置"
                  }}:</span
          ...
          <div class="blPadding">
            <div class="padList" :class="{ 'warning-alert': showWarning5 }">
              <div :class="{ active: jjdsTypeShow }">
                ...
                <div class="plIco twoIco"></div>
                <span class="plName"
                  >{{
                    listData?.g05?.findIndex((item) => item.id === g05Id) !== -1
                      ? g05Name
                      : "未配置"
                  }}:</span
                >
                <span class="plNum">{{
                  filterNum(
                    listData?.g05?.find((item) => item.id === g05Id)?.value
                      ? listData?.g05?.find((item) => item.id === g05Id)?.value
                      : "NA",
                    "g05"
                  )
                }}</span>
                <span class="plUnit fontsize70">万pc/m3 粒径0.5um</span>
              </div>
            </div>
          </div>
        </div>
          ...
          <div class="blPadding" style="padding-left: 20px">
            <div class="padList" :class="{ 'warning-alert': showWarning6 }">
              <div :class="{ active: jjdTypeShow }">
                ...
                <span class="plName"
                  >{{
                    listData?.g50?.findIndex((item) => item.id === g50Id) !== -1
                      ? g50Name
                      : "未配置"
                  }}:</span
                >
                <span class="plNum">{{
                  filterNum(
                    listData?.g50?.find((item) => item.id === g50Id)?.value
                      ? listData?.g50?.find((item) => item.id === g50Id)?.value
                      : "NA",
                    "g50"
                  )
                }}</span>
                <span class="plUnit fontsize70">万pc/m3 粒径5um</span>
              </div>
            </div>
          </div>
        </div>
    ...
    <div class="clickModule" v-if="showDetail">
      ...
      <div class="cmModule">
        <div class="cmUl">
          <div class="closeBtn">
            <span>报警屏蔽按钮</span>
            <span class="cl01FF00" @click="openBtn" v-if="!closeBtnType"
              >开启</span
            >
          ...
          <div class="blank33">
            <div class="cmList">
              <span
                >洁净度报警上限<label style="font-size: 16px">(0.5um)</label
                >:</span
              >
            ...
            <div class="cmList">
              <span
                >洁净度报警下限<label style="font-size: 16px">(0.5um)</label
                >:</span
              >
            ...
            <div class="cmList">
              <span
                >洁净度报警上限<label style="font-size: 16px">(5um)</label
                >:</span
              >
            ...
            <div class="cmList">
              <span
                >洁净度报警下限<label style="font-size: 16px">(5um)</label
                >:</span
              >
  ...
  <div class="plTime">
    <span>{{ nowDate }}</span> <span>{{ nowTime }}</span>
  </div>
  <div class="corporateName">深圳市森辉智能自控技术有限公司提供技术支持</div>
</template>
...
export default defineComponent({
  ...
  watch: {
    listData: function (newData, oldData) {
      ...
      let dew_point_c = this.listData?.dew_point_c?.find(
        (item) => item.id === this.dew_point_cId
      );
      let temperature = this.listData?.temperature?.find(
        (item) => item.id === this.temperatureId
      );
      let dif_pressure_pa = this.listData?.dif_pressure_pa?.find(
        (item) => item.id === this.dif_pressure_paId
      );
      let humidness = this.listData?.humidness?.find(
        (item) => item.id === this.humidnessId
      );

    //   console.log('this.listData?.g05',this.listData)
      ...
      if (
        (this.alarmsldWarning &&
          alarmsldup &&
          dew_point_c?.value > alarmsldup) ||
        (this.alarmsldWarning &&
          alarmsldlow &&
          dew_point_c?.value < alarmsldlow)
      ) {
      ...
      if (
        (this.alarmswdWarning &&
          alarmswdup &&
          temperature?.value > alarmswdup) ||
        (this.alarmswdWarning &&
          alarmswdlow &&
          temperature?.value < alarmswdlow)
      ) {
      ...
      if (
        (this.alarmsycWarning &&
          alarmsycup &&
          dif_pressure_pa?.value > alarmsycup) ||
        (this.alarmsycWarning &&
          alarmsyclow &&
          dif_pressure_pa?.value < alarmsyclow)
      ) {
      ...
      if (
        (this.alarmssdWarning && alarmssdup && humidness?.value > alarmssdup) ||
      ...
      let alarmsjjdsup =
        this.alarmsjjdsup == "" ? "" : Number(this.alarmsjjdsup);
      let alarmsjjdslow =
        this.alarmsjjdslow == "" ? "" : Number(this.alarmsjjdslow);
      if (
        (this.alarmsjjdsWarning && alarmsjjdsup && g05?.value > alarmsjjdsup) ||
      ...
      let alarmsjjdlow =
        this.alarmsjjdlow == "" ? "" : Number(this.alarmsjjdlow);
      if (
        (this.alarmsjjdlowWarning && alarmsjjdup && g50?.value > alarmsjjdup) ||
  ...
  methods: {
    ...
    filterNum(value: any, name: string) {
      let comparison = Number(value);
      value = Number(value);
      if (isNaN(value)) {
        return "NA";
      }
      value = this.keepTwoDecimalFull(Number(value));
      if (name === "ld") {
        if (value == "NA") {
          return "NA";
        }
        ...
        } else if (comparison >= this.ldSet.up) {
      ...
      if (name === "wd") {
        ...
        if (comparison <= this.wdSet.low) {
          return this.wdSet.low;
        } else if (comparison >= this.wdSet.up) {
      ...
      if (name === "yc") {
        ...
        if (comparison <= this.ycSet.low) {
          return this.ycSet.low;
        } else if (comparison >= this.ycSet.up) {
      ...
      if (name === "sd") {
        ...
        if (comparison <= this.sdSet.low) {
          return this.sdSet.low;
        } else if (comparison >= this.sdSet.up) {
    ...
    addWarningMsgList(index: never) {
      if (this.audioStatus === 0) {
        ...
        // 正在播放
        if (
          this.warningMsgList.length == 1 &&
          this.audioObj["src"] != this.audioFilePath[index]
        ) {
          this.warningMsgList.push(index);
        } else if (
          this.warningMsgList.length >= 1 &&
          this.warningMsgList[this.warningMsgList.length - 1] != index
        ) {
          this.warningMsgList.push(index);
        }
      }
        ...
        .then((data) => {
          try {
            ...
            if (data?.status === 200 && data?.data?.code === 200) {
              ...
              this.listData.dashboard_name = JSON.parse(
                result.content
              ).dashboard_name;
                ...
                result.forEach((item: any) => {
                  switch (item.conType) {
                    ...
                    case 5:
                      ...
                      this.alarmsjjdsup = item.alarmSuperiorLimit;
                      this.alarmsjjdslow = item.alarmLowerLimit;
                      this.alarmsjjdsWarning =
                        item.isAlarm === 1 ? true : false;
...
.closeIco {
  ...
  width: 38px;
  height: 38px;
  /* background-image: url("/image/closeIco.png"); */
  background: url("@/assets/image/closeIco.png");
...
.oneIco {
  background: url("@/assets/image/oneIco.png") no-repeat;
  background-size: 100% 100%;
}

.twoIco {
  background: url("@/assets/image/twoIco.png") no-repeat;
  background-size: 100% 100%;
}

.yc {
  background: url("@/assets/image/yc.png") no-repeat;
  background-size: 100% 100%;
}

.sd {
  background: url("@/assets/image/sd.png") no-repeat;
  background-size: 100% 100%;
}
