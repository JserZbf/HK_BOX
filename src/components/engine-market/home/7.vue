	...
	<div class="height" @click="clickTab">
		<div class="flex-row height">
			<div class="boardCenter">
				<div  v-for="(item,i) in 7" :key="i">
				<div class="bcList title">
					<div class="blPadding">
						<div class="padList" :class="{       'warning-alert':       showWarning1     }">
							<div :class="{ active: ldTypeShow }">
								<span class="plName">{{ title1 }}</span>
							</div>
						</div>
					</div>
				</div>
						...
						<div class="padList" :class="{       'warning-alert':       showWarning1     }">
							<div :class="{ active: ldTypeShow }">
								<div>
									...
									<span class="plName">粒子0.5μm:</span>
								</div>
							
								<span
									class="plNum">{{ filterNum(listData?.dew_point_c.find(item=>item.id===dew_point_cId)?.value?listData?.dew_point_c.find(item=>item.id===dew_point_cId)?.value:'NA','ld') }}</span>
								<span class="plUnit">万pc/m3</span>
							</div>
						</div>
					</div>
				</div>
					...
					<div class="blPadding">
						<div class="padList" :class="{       'warning-alert':       showWarning2      }">
							<div :class="{ active: wdTypeShow }">
								<div>
									<img class="plIco" src="../../../assets/image/twoIco.png" />
									<span class="plName">粒子5μm:</span>
								</div>
								<span
									class="plNum">{{ filterNum(listData?.temperature.find(item=>item.id===temperatureId)?.value?listData?.temperature.find(item=>item.id===temperatureId)?.value:'NA','wd') }}</span>
								<span class="plUnit">万pc/m3</span>
